...
| Rota | Tipo | Conteúdo |
|------|------|----------|
| `/(tabs)/settings` | aba | Programa · Tipo de sequência · Agenda semanal (só `WEEKLY`) · Reiniciar sequência (só `CONTINUOUS`) |
...
- **Tipo de sequência**: sempre permitido, inclusive com sessão em andamento. Escolher `WEEKLY` num programa sem agenda é permitido; a Home orienta (spec 004) e a linha "Agenda semanal" leva ao estado vazio.
